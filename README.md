...
<div align="center">
https://github.com/user-attachments/assets/02effa25-8796-431b-95a1-a41886d8b59c
</div>
...
- 📑 **マルチページファイルプレビュー** - PDF/DOCX/XLSXの全ページをインライン表示、前後ページナビゲーション対応
...
- 📰 **ニュース機能** - newsdata.ioから日本のニュース記事を取得・閲覧し、AIとニュースについて対話
...
4. **マルチページファイル表示**: PDF/DOCX/XLSXは全ページが表示され、「前へ」「次へ」ボタンでページ間を移動可能
5. **モデル選択**: ヘッダーのドロップダウンから使用するモデルを選択
6. **モデル比較モード**: ヘッダーの比較アイコン（GitCompare）をクリックして2-4個のモデルを選択し、同時に応答を比較
7. **新しいチャット**: サイドバーの「+」ボタン、またはヘッダーのタイトル「Ollama Chat」をクリックして新しいチャットセッションを作成
8. **チャット履歴**: サイドバーから過去のチャットセッションを選択して閲覧
9. **検索**: サイドバーの検索アイコンでチャット履歴を検索
10. **生成のキャンセル**: メッセージ生成中にキャンセルボタンをクリックして中断（キャンセルされたメッセージは記録され、入力内容は復元されます）
11. **長いメッセージの表示**: 6行を超えるメッセージは自動的に折りたたまれ、展開/折りたたみボタンで表示を切り替え可能
12. **フィードバック送信**: アシスタントの応答メッセージに👍（良い）または👎（悪い）ボタンでフィードバックを送信
13. **統計情報の確認**: サイドバーの「統計情報」からモデル別の使用状況やフィードバック統計を確認
14. **プロンプトテンプレート**: チャット画面でCmd/Ctrl+Jを押すか、FABボタンをクリックして保存済みテンプレートを素早く適用

### 高度な機能

- **モデル管理**: `/models` ページまたはモデルセレクターからモデルのダウンロード、削除、詳細確認が可能
- **クラウドモデル連携**: モデルセレクターからGemini、GPT、Claude、Grokなどクラウドモデル用のAPIキーを登録・管理し、チャットで利用可能
- **AIモデル比較モード**:
  - 2-4個のモデルを選択して同時に同じプロンプトで応答を生成
  - 応答時間、トークン数（プロンプト/生成）をリアルタイム表示
  - グリッドレイアウトで並べて比較（2列、3列、4列に自動調整）
...
- **マルチページファイルビューア**:
  ...
  - 現在のページ番号と総ページ数を表示
- **ノート機能**: `/notes` ページでチャット内容やアイデアをノートとして保存、閲覧、検索、ラベル管理
- **ファイル管理**: `/files` ページでアップロードしたファイルの一覧を確認
- **ニュース閲覧**: `/news` ページで日本の最新ニュースを閲覧し、記事についてAIと対話可能（newsdata.io APIキーが必要）
- **URL取得**: チャット入力欄のリンクアイコンからウェブページのURLを入力し、コンテンツを取得してチャットで利用
...
- **プロンプトテンプレート**: `/prompts` ページで再利用可能なプロンプトを作成・管理
  - テンプレート名、説明、プロンプトテキスト、カテゴリを設定
  - お気に入り登録でよく使うテンプレートを素早くアクセス
  - カテゴリ別フィルタリングと検索機能
  - チャット画面からCmd/Ctrl+Jまたは青いFABボタンでテンプレート選択モーダルを開く
  - 選択したテンプレートは現在の入力に自動的に挿入され、即座に使用可能
  - 使用回数を自動追跡
...
- **サービス層**: ビジネスロジックをサービスクラスに分離（`services/`）
  - `ChatService`: チャット処理のオーケストレーション（Ollama/クラウドルーティング、トークン数追跡）
  ...
  - `cloud_providers/`: クラウドAPIプロバイダー
    ...
    - `GPTProvider`: OpenAI GPT API実装
    - `ClaudeProvider`: Anthropic Claude API実装
    - `GrokProvider`: xAI Grok API実装
...
│   │   │   ├── ComparisonView.tsx       # モデル比較グリッド表示
...
│   │   │   ├── ModelList.tsx
│   │   │   ├── ModelSelector.tsx
│   │   │   ├── MultiModelSelector.tsx   # モデル比較用複数選択
│   │   │   ├── ModelStatsModal.tsx
│   │   │   ├── NoteCreateModal.tsx
│   │   │   ├── NoteDetailModal.tsx
│   │   │   ├── NoteList.tsx
